...
if Mtype > 0
  ...
  Msingular = odeget(options,'MassSingular','maybe');
  switch Msingular
    case 'no',     DAE = false;
    case 'yes',    DAE = true;
  ...
  if DAE
    ...
    if isempty(yp0)
      ...
      if odeget(options,'SkipICCheck',false)
        yp0_OK = true;
      else
...
if Jconstant 
  if isempty(Jac) % use odenumjac
    [Jac,Joptions.fac,nF] = odenumjac(odeFcn, {t0,y0,odeArgs{:}}, f0, Joptions); %#ok<CCAT>
    nfevals = nfevals + nF;
    npds = npds + 1;
...
while ~done
  ...
  calculate_jacobian_every_step = true;
  if calculate_jacobian_every_step
    if Janalytic
      dfdy = feval(Jac,t,y,Jargs{:});
      % save('dfdy.mat', 'dfdy')
    else
  ...
  while true                            % Evaluate the formula.
    ...
    while ~gotynew
      ...
      if normcontrol
        normynew = norm(ynew);
        invwt = 1 / max(max(normy,normynew),thres);
        minnrm = 100*eps*(normynew * invwt);
      else
        invwt = 1 ./ max(max(abs(y),abs(ynew)),thres);
